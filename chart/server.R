...
shinyServer <- function(input, output, session) {
  ...
  FullData <- read.csv("data/FPDS_data.csv")
  ...
  dataset <- reactive({
    if (input$color_var == 'Category') {
      ...
      FullData$Competition <- fct_recode(FullData$Competition,
      ...
      FullData <-
        filter(FullData,
               Platform_Portfolio != "NULL" & !is.null(Platform_Portfolio))
      FullData$Platform_Portfolio <-
        fct_drop(FullData$Platform_Portfolio)
  ...
  lineplotsetting <- reactive({
    ...
    if (min(dataset()$Percent > 0)) {
      ...
      mainplot <- mainplot +
        coord_cartesian(ylim = c(
          1.05 * min(dataset()$Percent),
          1.05 * max(dataset()$Percent)
        ))
    }
    ...
    if (input$color_var == 'Platform_Portfolio') {
      mainplot +
        ...
        scale_x_continuous(
          breaks = seq(input$Yr[1], input$Yr[2], by = 2),
          labels = function(x) {
            substring(as.character(x), 3, 4)
          }
        ) +
  ...
  barplotsetting <- reactive({
    ...
    mainplot <- mainplot +
      ...
      facet_wrap(
        as.formula(paste0("~ `", input$color_var, "`")),
        nrow = numrow,
        scales = "free_x",
        drop = TRUE
      ) +
  ...
  output$hover_info <- renderUI({
    ...
    if (input$Chart == "Bar") {
      ...
      if (hover$y > 0 & hover$y > hov_amount)
        return(NULL)
      # end of bar
    } else {
      year <- point$FY
